...
/**
 * This class is used for indexing promotionDiscount of variants for the corresponding products.
 *
 * @author ravindra.kr
 *
 */
public class SparVariantsPromotionDiscountValueProvider extends AbstractPropertyFieldValueProvider implements Serializable,
		FieldValueProvider
{
	private static final String ERROR_MESSAGE = "Cannot evaluate variant's price of non-product item";
	private static final String PROMOTION_DISCOUNT = "promotionDiscount";
	private FieldNameProvider fieldNameProvider;
	private PriceService priceService;
	private static final String WAREHOUSE = "warehouse";

	/**
	 * This method is used to get the FieldValue for promotionDiscount of a variant's price and push it into Solr
	 */
			...
			final IndexedProperty indexedProperty, final IndexConfig indexConfig)
	{
		...
		for (final VariantProductModel variantProductModel : variants)
		{
			if (variantProductModel instanceof SparVariantProductModel)
			{
				...
				String promotionDiscountValue = null;
					...
					try
					{
						...
						if (!CollectionUtils.isEmpty(prices))
						{
							...
							for (final Object obj : prices)
							{
								...
								if(null != (price).getQualifiers().get(PROMOTION_DISCOUNT))
								{
									value = (Double) (price).getQualifiers().get(PROMOTION_DISCOUNT);
								}
								...
								promotionDiscountValue = promotionDiscountValue + warehouse + SparCoreConstants.SparValueProviderConstants.FIELD_VALUE_DELIMETER
										+ value + SparCoreConstants.SparValueProviderConstants.POS_LEVEL_DELIMETER;
							}
							addFieldValues(indexedProperty, fieldValues, promotionDiscountValue);
						}
						else
						{
							...
							if (baseSiteModel != null && baseSiteModel.getStores() != null && !baseSiteModel.getStores().isEmpty())
							{
								...
								{
									promotionDiscountValue = promotionDiscountValue + warehouse.getCode()
