...
/**
 * This class is used to index combi offer (indexproperty) for base products that are warehouse specific in Solr.
 *
 *
 */
public class SparProductCombiOfferValueProvider extends ProductPriceValueProvider
{
			...
			final Object model) throws FieldValueProviderException
	{
		...
		try
		{
			...
			// To check all variants values combi offer field of the product
			...
			final List prices) throws FieldValueProviderException
	{
		...
		for (final Object field : fieldNames)
		{
			...
			for (final Object obj : prices)
			{
				...
				{
					if (null != (price).getQualifiers().get(COMBI_OFFER))
					{
						benefitType = (price).getQualifiers().get(COMBI_OFFER).toString();
						break;
					}
				}
			}
