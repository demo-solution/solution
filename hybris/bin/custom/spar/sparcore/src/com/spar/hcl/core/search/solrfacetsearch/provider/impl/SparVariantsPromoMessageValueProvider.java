...
/**
 * This class is used for indexing PROMO Message of variants for the corresponding products.
 *
 * @author rohan_c
 *
 */
public class SparVariantsPromoMessageValueProvider extends AbstractPropertyFieldValueProvider implements Serializable,
		FieldValueProvider
{
	private static final String PROMO_MESSAGE = "promoMesssage";
	...
	/**
	 * This method is used to get the FieldValue for promo message of a variants and push it into Solr
	 */
					...
					try
					{
						...
						if (!CollectionUtils.isEmpty(prices))
						{
							...
							for (final Object obj : prices)
							{
								...
								value = (String) (price).getQualifiers().get(PROMO_MESSAGE);
								value = value == null ? "" : value;
								final Warehouse priceWarheouse = (Warehouse) (price).getQualifiers().get(WAREHOUSE);
								promoMessageValue = promoMessageValue + priceWarheouse.getCode()
							...
							addFieldValues(indexedProperty, fieldValues, promoMessageValue);
						}
						else
						{
							...
							if (baseSiteModel != null && baseSiteModel.getStores() != null && !baseSiteModel.getStores().isEmpty())
							{
								...
								{
									promoMessageValue = promoMessageValue + warehouse.getCode()
	...
	/**
	 * This method is used to add Promo Message for variants in Solr
	 *
	 * @param indexedProperty
	 * @param fieldValues
	 * @param priceValue
