...
/**
 * This class is used for indexing productOnBogo of variants for the corresponding products.
 *
 * @author ravindra.kr
 *
 */
public class SparVariantsProductOnBogoValueProvider extends AbstractPropertyFieldValueProvider implements Serializable,
		FieldValueProvider
{
	private static final String ERROR_MESSAGE = "Cannot evaluate variant's price of non-product item";
	private static final String PRODUCT_ON_BOGO = "productOnBogo";
	private FieldNameProvider fieldNameProvider;
	private PriceService priceService;
	private static final String WAREHOUSE = "warehouse";

	/**
	 * This method is used to get the FieldValue for price's productOnBogo of a variants and push it into Solr
	 */
			...
			final IndexedProperty indexedProperty, final IndexConfig indexConfig)
	{
		...
		for (final VariantProductModel variantProductModel : variants)
		{
			if (variantProductModel instanceof SparVariantProductModel)
			{
				...
				Boolean value = null ;
				String productOnBogoValue = null;
					...
					try
					{
						...
						if (!CollectionUtils.isEmpty(prices))
						{
							...
							for (final Object obj : prices)
							{
								...
								if(null != (price).getQualifiers().get(PRODUCT_ON_BOGO))
								{
									value = (Boolean) (price).getQualifiers().get(PRODUCT_ON_BOGO);
								}
								...
								productOnBogoValue = productOnBogoValue + warehouse + SparCoreConstants.SparValueProviderConstants.FIELD_VALUE_DELIMETER
										+ value + SparCoreConstants.SparValueProviderConstants.POS_LEVEL_DELIMETER;
							}
							addFieldValues(indexedProperty, fieldValues, productOnBogoValue);
						}
						else
						{
							...
							if (baseSiteModel != null && baseSiteModel.getStores() != null && !baseSiteModel.getStores().isEmpty())
							{
								...
								{
									productOnBogoValue = productOnBogoValue + warehouse.getCode()
