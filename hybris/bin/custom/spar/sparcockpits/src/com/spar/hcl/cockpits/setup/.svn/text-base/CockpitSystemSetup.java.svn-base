 ...
 *
 */
package com.spar.hcl.cockpits.setup;
...
import com.spar.hcl.cockpits.constants.SparCockpitsConstants;
 ...
 */
@SystemSetup(extension = SparCockpitsConstants.EXTENSIONNAME)
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(SparCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(SparCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						SparCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				if (prefix != null){
					importImpexFile(context, "/sparcockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
				}
			}
