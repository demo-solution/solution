...
<div class="col-md-3 col-sm-4 col-xs-6 separator">
	<div class="product-image-wrapper">
		...
		<c:set var="baseVariantOption" value="${variantOption}" />
							...
							<%-- <option selected="selected" disabled="disabled"><spring:theme code="product.variants.select.variant"/></option> --%>
								...
								<c:if test="${baseVariantOption.code ne variantOption.code }">
									...
									<c:url value="${variantOption.url}" var="variantOptionUrl"/>
										<option value="${variantOption.code}" selected="selected">
									</c:if>
									<c:if test="${variantOption.code ne product.code}">
			...
			<ycommerce:testId code="product_productPrice">
				...
				<c:choose>
				 	...
				 	<c:otherwise>
					 	<c:choose>
							<c:when test="${baseVariantOption.priceData.value < baseVariantOption.unitMRP.value}">
								<c:choose>
									<c:when test="${product.isProductShownMP}">
										<span class="prev-price">MP &nbsp; <format:price priceData="${baseVariantOption.unitMRP}"/></span>
									</c:when>
									<c:otherwise>
										<span class="prev-price">MRP &nbsp; <format:price priceData="${baseVariantOption.unitMRP}"/></span>
									</c:otherwise>
								</c:choose>
							</c:when>
							<c:otherwise>
					...
					<span class="orignal-price"><format:price priceData="${baseVariantOption.priceData}"/></span>
					<c:choose>
