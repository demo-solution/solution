...
<div class="row search-results nomargin clearfix">
<c:if test="${searchPageData.pagination.totalNumberOfResults > 0}">

			<div class="col-sm-7 col-md-7 col-lg-7 col-xs-5 nopadding">
				<ycommerce:testId code="searchResults_productsFound_label">
                    <c:choose>
			...
			<div class="col-sm-5 col-md-5 col-lg-5 col-xs-7 nopadding">
			...
			<c:if test="${not empty searchPageData.sorts}">
				...
				<c:if test="${ top}">
						<div class="sortform col-xs-9 col-sm-9 col-md-9 nopadding">
								<form id="sortForm${top ? '1' : '2'}" name="sortForm${top ? '1' : '2'}" method="get" action="#">
									<select id="sortOptions${top ? '1' : '2'}" name="sort" class="form-control pull-right col-sm-3" style="margin-right:0px;">
