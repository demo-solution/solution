ACC.productDetail = {
	...
	updateQtyValue : function(self, value) {		
		var maxOrderQuantity = $('.priceTextsBG .productMaxOrderQuantity').val();
		var input = $(self).parents(".js-qty-selector").find(
				".js-qty-selector-input");
		...
		$("#qty").val(value);
		if (parseInt(value) >= parseInt(maxOrderQuantity)) {
			$('.js-qty-selector-plus').attr("disabled", "disabled");
		} else if (parseInt(value) <= parseInt(maxOrderQuantity)) {
			$('.js-qty-selector-plus').removeAttr("disabled");
		} else {
			$('.js-qty-selector-plus,.js-qty-selector-minus').removeAttr(
					"disabled");
		}
		$('.update-entry-quantity-input').blur();
	},
	initPageEvents : function() {
		...
		$(document).on(
				...
				'.js-qty-selector .js-qty-selector-input',
				function(e) {
					var userInputVal=parseInt($(this).val());
					var maxOrderQuantity =parseInt($('.priceTextsBG .productMaxOrderQuantity').val());
					var maxProductQuantity = parseInt($('.priceTextsBG .maxProductQuantity').val());
					
					if (userInputVal <= maxOrderQuantity && userInputVal <= maxProductQuantity) {							
						$(this).val(userInputVal);						
					}else if(userInputVal <= maxOrderQuantity && userInputVal >= maxProductQuantity){						
						$(this).val(maxProductQuantity);
					}else{						
