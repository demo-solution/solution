...
{
	private final static Logger LOG = Logger.getLogger(OrdersHelper.class);
			...
			final String sort, final String fields)
	{
		...
		if (CollectionUtils.isNotEmpty(orderHistoriesData.getOrders()))
		{
			for (final OrderHistoryData orderData : orderHistoriesData.getOrders())
			{
				for (final OrderHistoryWsDTO orderHistoryData : dto.getOrders())
				{
					if (orderData.getCode().equals(orderHistoryData.getCode()))
					{
						orderHistoryData.setDeliveryDate(orderData.getDelivered());
						break;
					}
