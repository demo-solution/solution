 ...
 *
 */
package com.spar.hcl.populator;
		...
		Populator<HttpServletRequest, AddressData>
{
	...
	public void populate(final HttpServletRequest request, final AddressData addressData)
	{
		...
		addressData.setArea(updateStringValueFromRequest(request, AREA, addressData.getArea()));
		addressData.setBuildingName(updateStringValueFromRequest(request, BUILDINGNAME, addressData.getBuildingName()));
		...
		addressData.setLandmark(updateStringValueFromRequest(request, LANDMARK, addressData.getLandmark()));
	}
	...
	protected CountryData updateCountryFromRequest(final HttpServletRequest request, final CountryData defaultValue)
	{
		...
		if (StringUtils.isNotBlank(countryIsoCode))
		{
			...
			try
			{
				countryDataFromFacade = i18nFacade.getCountryForIsocode(countryIsoCode);
			}
