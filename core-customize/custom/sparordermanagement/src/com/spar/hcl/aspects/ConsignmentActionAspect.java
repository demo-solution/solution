...
public class ConsignmentActionAspect
{
	 ...
	 */
	public Collection<OrderCancelEntry> advise(final ProceedingJoinPoint joinPoint) throws Throwable
	{
		Collection<OrderCancelEntry> remainingEntries = Collections.emptyList();
		...
		try
		{
			getConsignmentBusinessProcessService().triggerChoiceEvent(consignment,
					SparordermanagementConstants.CONSIGNMENT_ACTION_EVENT_NAME, getChoice());
			remainingEntries = (Collection<OrderCancelEntry>) joinPoint.proceed();
	...
	protected void triggerAfter(final ConsignmentModel consignment)
	{
		getConsignmentBusinessProcessService().triggerSimpleEvent(consignment,
				SparordermanagementConstants.ACTION_COMPLETION_EVENT_NAME);
	}
