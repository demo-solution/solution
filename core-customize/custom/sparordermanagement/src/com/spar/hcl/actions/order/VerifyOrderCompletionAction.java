...
public class VerifyOrderCompletionAction extends AbstractSimpleDecisionAction<OrderProcessModel>
{
	...
	public Transition executeAction(final OrderProcessModel process)
	{
		...
		final boolean someEntriesShipped = order.getEntries().stream().anyMatch(entry -> ((OrderEntryModel)entry).getQuantityShipped() > 0);
		...
		{
			final boolean someEntriesWaiting = order.getEntries().stream().anyMatch(entry ->((OrderEntryModel)entry).getQuantityPending() > 0);
