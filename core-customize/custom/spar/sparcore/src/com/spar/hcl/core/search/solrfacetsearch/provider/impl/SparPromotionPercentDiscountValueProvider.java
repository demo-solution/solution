 ...
 *
 */
public class SparPromotionPercentDiscountValueProvider extends ProductPriceValueProvider
{
			...
			final List prices) throws FieldValueProviderException
	{
		...
		for (final Object field : fieldNames)
		{
			...
			for (final Object obj : prices)
			{
				...
				{
					if (null != (price).getQualifiers() && null != (price).getQualifiers().get(PROMOTION_DISCOUNT))
					{
						promotionDiscount = (price).getQualifiers().get(PROMOTION_DISCOUNT).toString();
						break;
					}
				}
			}
			...
			{
				fieldValues.add(new FieldValue(fieldName, promotionDiscount));
			}
		}
	}
