...
/**
 * This class is used to index Name for variant products childSKUs in Solr
 *
 * @author ravindra.kr
 *
 */
public class SparVariantsChildSKUsValueProvider extends AbstractPropertyFieldValueProvider implements Serializable, FieldValueProvider
{
	private static final String ERROR_MESSAGE = "Cannot evaluate Child SKUs of non-product item";
			...
			final IndexedProperty indexedProperty)
	{
		...
		for (final VariantProductModel variantProductModel : variants)
		{
			...
			{
				final String childSKU = ((SparVariantProductModel) variantProductModel).getChildSKUs();
				childSKUs.add(childSKU);
				addFieldValues(fieldValues, indexedProperty, childSKUs);
			}
		}
		return fieldValues;
	}
			...
			final List<String> fieldValue)
	{
		...
		{
			for (final String condition : fieldValue)
			{
