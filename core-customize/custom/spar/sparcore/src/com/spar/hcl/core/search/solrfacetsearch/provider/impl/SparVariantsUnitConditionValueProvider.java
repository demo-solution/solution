...
/**
 * This class is used for indexing variants for the corresponding products.
 *
 * @author rohan_c
 *
 */
public class SparVariantsUnitConditionValueProvider extends AbstractPropertyFieldValueProvider implements Serializable,
		FieldValueProvider
{
			...
			final IndexedProperty indexedProperty)
	{
		...
		final List<String> unitConditions = new ArrayList();
		for (final VariantProductModel variantProductModel : variants)
		{
			if (variantProductModel instanceof SparVariantProductModel)
			{
				unitConditions.add(((SparVariantProductModel) variantProductModel).getUnitCondition());
			}
		}
		addFieldValues(fieldValues, indexedProperty, unitConditions);
		return fieldValues;
	}
