 ...
 *
 */
public class SparProductOnBogoValueProvider extends ProductPriceValueProvider
{
			...
			final List prices) throws FieldValueProviderException
	{
		...
		for (final Object field : fieldNames)
		{
			...
			for (final Object obj : prices)
			{
				...
				{
					if (null != (price).getQualifiers() && null != (price).getQualifiers().get(PRODUCT_ON_BOGO))
					{
						productOnBogo = (price).getQualifiers().get(PRODUCT_ON_BOGO).toString();
						break;
					}
				}
			}
			fieldValues.add(new FieldValue(fieldName, productOnBogo));
		}
	}
