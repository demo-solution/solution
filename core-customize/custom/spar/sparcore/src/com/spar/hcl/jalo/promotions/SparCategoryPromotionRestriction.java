...
public class SparCategoryPromotionRestriction extends GeneratedSparCategoryPromotionRestriction
{
	private final ModelService modelService = (ModelService) Registry.getApplicationContext().getBean("modelService");

	@SuppressWarnings("unused")
	private final static Logger LOG = Logger.getLogger(SparCategoryPromotionRestriction.class.getName());
			...
			final AbstractOrder order)
	{
		if (order != null)
		{
			...
			{
				if (promotionRestriction instanceof SparCategoryPromotionRestriction)
				{
					double totalCategoryItemPrice = 0.0;
					final Collection<Category> categories = ((SparCategoryPromotionRestriction) promotionRestriction).getCategories();
