 ...
 *
 */
package com.spar.hcl.storefront.filters.cms;
...
public class CMSSiteFilter extends OncePerRequestFilter implements CMSFilter
{
	...
	public void setDefaultPointsOfService()
	{
		// There should be an additional check to retrieve POS if Cart exist.
		final SessionService sessionService = getSessionService();
		String selectedStore = sessionService.getAttribute("selectedStore");
		if (null == selectedStore)
		{
			selectedStore = storeFinderFacadeInterface.getDefaultStore().getName();
	...
	public void getDeliveryCityName()
	{
		final SessionService sessionService = getSessionService();

		if(null == sessionService.getAttribute("cityName"))
		{
   		final String selectedCity = storeFinderFacadeInterface.getDefaultStore().getAddress().getTown();
   		sessionService.setAttribute("cityName", selectedCity);
		}
	}
