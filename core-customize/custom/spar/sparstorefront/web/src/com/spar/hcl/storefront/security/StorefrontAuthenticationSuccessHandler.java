...
public class StorefrontAuthenticationSuccessHandler extends SavedRequestAwareAuthenticationSuccessHandler
{
	...
	private de.hybris.platform.acceleratorstorefrontcommons.security.BruteForceAttackCounter bruteForceAttackCounter;
			...
			final Authentication authentication) throws IOException, ServletException
	{
		...
		getLRDetailsForCustomer(request);
	}
	...
	private void getLRDetailsForCustomer(final HttpServletRequest request)
	{
		final HttpSession session = request.getSession();
		final CustomerData customerData = customerFacade.getCurrentCustomer();
		if(null != customerData.getCustPrimaryMobNumber() && customerData.getIsOTPValidate() 
				&& !customerData.getDisableLRPrompt() && !customerData.getLrOptStatus())
		{
			if(null != customerData.getDateOfBirth() && null != customerData.getDefaultBillingAddress())
			{
      			String sysGenOTPKey = RandomStringUtils.randomNumeric(6);
      			try
      			{
      				sparCustomerFacade.getmGageOTPResponse(sysGenOTPKey, customerData.getCustPrimaryMobNumber());
     				}
     				catch (IOException e)
     				{
      			...
      			//getSessionService().setAttribute("otpLR", sysGenOTPKey);
      			session.setAttribute("otpLR", sysGenOTPKey);
      			session.setAttribute("mobNumberForLRPopup", customerData.getCustPrimaryMobNumber());
			}
		}
	}
}
