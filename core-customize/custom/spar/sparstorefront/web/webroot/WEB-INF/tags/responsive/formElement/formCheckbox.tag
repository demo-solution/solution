...
<template:errorSpanField path="${path}">
	<spring:theme code="${idKey}" var="themeIdKey"/>
	   	...
	   	<div class="checkbox">
	   	<c:choose>
	   		<c:when test="${themeIdKey == 'saveAddressInMyAddressBook' || themeIdKey == 'defaultAddress'}">
		   		<label class="control-label ${labelCSS}" for="${themeIdKey}">
		   			<form:checkbox cssClass="${inputCSS}" id="${themeIdKey}" path="${path}" tabindex="${tabindex}" checked="checked"/>
		   		...
		   		<c:when test="${idKey == 'register.whetherSubscribedToLandmark'}">
		   		<label class="control-label ${labelCSS}" for="${themeIdKey}">
